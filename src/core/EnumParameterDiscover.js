var EnumParameterDiscover = (function () {
	var OPERATION = "enum.parameter.discover";
	var MAX_DEPTH = 8;
	...
	function looksLikeVideoClipComponent(displayName, matchName, mediaType) {
		var media = fold(mediaType);
		if (media === "video") {
			return {
				matched: true,
				reason: "component on Video TrackItem"
			};
		}
	...
	function jsonSafeOptionValue(value) {
	...
	function isBlendModeName(snapshot) {
		return fold(snapshot && snapshot.displayName) === "blend mode";
	}

	function looksEnumLike(snapshot) {
		return !!(snapshot && (snapshot.enumLike === true || isBlendModeName(snapshot)));
	}

	function representationOf(snapshot) {
		var type = snapshot && snapshot.runtimeType ? snapshot.runtimeType : "unknown";
		var hasMeta = !!(snapshot && snapshot.enumMetadata && snapshot.enumMetadata.hasSafeOptions);
		var valueType = snapshot ? typeof snapshot.currentValue : "undefined";
		if ((type === "number" || type === "enum") && valueType === "number" && hasMeta) {
			return "E";
		}
		if (type === "enum") {
			if (valueType === "number") {
			...
			if (snapshot.currentValue && valueType === "object") {
				return "C";
			}
			return "D";
		}
		...
		if (type === "object" || (snapshot && snapshot.valueShape && snapshot.valueShape.kind === "object")) {
			return "C";
		}
		if ((type === "number" || valueType === "number") && !hasMeta) {
			return "F";
		}
		if (hasMeta) {
			return "D";
	...
	function classifySnapshot(snapshot) {
		var type = snapshot && snapshot.runtimeType ? snapshot.runtimeType : "unknown";
		var meta = snapshot && snapshot.enumMetadata;
		var hasMeta = !!(meta && meta.hasSafeOptions);
		var representation = representationOf(snapshot);
		var valueType = snapshot ? typeof snapshot.currentValue : "undefined";
		if (snapshot && snapshot.timeVarying === true) {
			return {
				classification: "PARAMETER_TIME_VARYING_UNSUPPORTED",
				reason: "PARAMETER_TIME_VARYING_UNSUPPORTED",
				candidate: false,
				productionEnabled: false,
				status: "REJECTED",
		...
		if (hasMeta && (type === "string" || valueType === "string")) {
			return {
				classification: "STRING_ENUM",
				reason: undefined,
				candidate: true,
				productionEnabled: false,
				status: "CANDIDATE",
		...
		if (hasMeta && (type === "number" || valueType === "number")) {
			return {
				classification: "ENUM_LIKE_NUMBER",
				reason: "ENUM_LIKE_NUMBER",
				candidate: true,
				productionEnabled: false,
				status: "ENUM_LIKE_NUMBER",
				writeCapability: "ENUM",
				representation: "E"
			};
		}
		if (type === "enum" && hasMeta) {
			return {
				classification: "ENUM_CANDIDATE",
				reason: undefined,
				candidate: true,
				productionEnabled: false,
				status: "CANDIDATE",
		...
		if (hasMeta && (type === "unknown" || type === "boolean")) {
			return {
				classification: "ENUM_CANDIDATE",
				reason: undefined,
				candidate: true,
		...
		if ((type === "number" || valueType === "number") && looksEnumLike(snapshot) && !hasMeta) {
			return {
				classification: "ENUM_LIKE_NUMBER",
				reason: "ENUM_LIKE_NUMBER",
				candidate: false,
				...
				writeCapability: "ENUM",
				representation: "F",
				detail: "Numeric enum-like parameter without safely enumerable option values."
			};
		}
		if (looksEnumLike(snapshot) && !hasMeta) {
			return {
				classification: "MISSING_ENUM_METADATA",
		...
		return {
			classification: typeSpecificReason(snapshot, { reason: "UNSUPPORTED_TYPE", status: "UNSUPPORTED" }) || "UNSUPPORTED_TYPE",
			reason: "UNSUPPORTED_TYPE",
			candidate: false,
			productionEnabled: false,
			status: "REJECTED",
	...
	function publicSnapshot(node) {
		...
		row = {
			...
			metadataSources: node.enumMetadata ? node.enumMetadata.sources : undefined,
			enumValues: node.enumMetadata ? node.enumMetadata.options : undefined,
			representation: classified.representation,
			isTimeVarying: node.timeVarying === true,
			safelyWritable: classified.candidate === true &&
				!!(node.enumMetadata && node.enumMetadata.hasSafeOptions) &&
				node.timeVarying !== true,
			classifierUsed: "EnumParameterDiscover.classifySnapshot",
			writesPerformed: false
		};
	...
	function inspectNode(res, component, param, diagnosticIndex, parentInfo, depth, outLeaves) {
		...
		if (fold(displayName) === "blend mode") {
			node.enumLike = true;
		}
		node.enumMetadata = extractEnumMetadata(param);
		if (node.enumMetadata && node.enumMetadata.hasSafeOptions) {
			node.enumLike = true;
		}
		try {
	...
	function inspectComponent(res, component, outLeaves) {
		...
		var fakeRoot;
		var videoMatch = looksLikeVideoClipComponent(displayName, matchName, "Video");
		fakeRoot = {
			displayName: displayName,
			matchName: matchName,
		...
		return {
			displayName: displayName,
			matchName: matchName,
			videoRelated: videoMatch.matched,
			matchedBy: videoMatch.reason,
			nested: fakeRoot
		};
	}
	...
	function inspectTrackItem(trackItem) {
		...
		for (i = 0; i < countInfo.count; i++) {
			...
			displayName = res.readString(component, "displayName") || "";
			matchName = res.readString(component, "matchName") || "";
			videoMatch = looksLikeVideoClipComponent(displayName, matchName, mediaType);
			clipComponents.push({
				displayName: displayName,
				matchName: matchName,
				videoRelated: videoMatch.matched,
	...
	function buildReport(inspected) {
		...
		if (!inspected || inspected.ok !== true) {
			if (inspected && inspected.clipComponents) {
				...
				inspected.detail = inspected.detail || "No video component was found on the selected clip.";
		...
		return {
			...
			rejected: rejected,
			blendModeMatches: (function () {
				var list = [];
				var n;
				var name;
				for (n = 0; n < parameters.length; n++) {
					name = String(parameters[n].displayName || "").replace(/^\s+|\s+$/g, "").toLowerCase();
					if (name === "blend mode") {
						list.push(parameters[n]);
					}
				}
				return list;
			}()),
	...
	function resolutionMeta(ident, extra) {
		var chosen = extra || {};
		return {
			resolverUsed: "EnumParameterDiscover.resolve",
	...
	function resolve(trackItem, componentHint, parameterName, parameterMatchName) {
		...
		if (!ident.componentDisplayName && !ident.componentMatchName) {
			return resolveFail(
				"NO_SAFE_IDENTITY",
				"Enum write resolution requires component identity. Parameter displayName alone is not used.",
				ident
			);
		}
	...
	return {
		...
		looksLikeVideoClipComponent: looksLikeVideoClipComponent,
		candidatesOf: candidatesOf
	};
}());

if (typeof $ !== "undefined") {
	$._pickfxEnumParameterDiscover = EnumParameterDiscover;
}

if (typeof module !== "undefined" && module.exports) {
	module.exports = EnumParameterDiscover;
}
